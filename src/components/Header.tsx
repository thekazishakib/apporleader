...
import { effectiveRegistrationUrl } from '../config/siteSettingsMap';
...
export default function Header() {
  ...
  const { membershipUrl, siteSettings, isAdmin } = useAdmin();
  const joinUrl = effectiveRegistrationUrl(siteSettings);
  const showSeparateRegistration =
    siteSettings.registrationUrl.trim() !== '' &&
    siteSettings.registrationUrl.trim() !== siteSettings.membershipUrl.trim();
            ...
            <div className="flex items-center gap-3">
              ...
              {showSeparateRegistration && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 text-sm font-medium text-gray-300 border border-white/15 rounded-full hover:bg-white/10 transition-colors"
                >
                  Member
                </a>
              )}
              ...
              {isAdmin && (
                <Link
                  to="/admin"
                  className="px-3 py-2 text-xs font-semibold text-primary border border-primary/40 rounded-full hover:bg-primary/10 transition-colors"
                >
                  Admin
                </Link>
              )}
