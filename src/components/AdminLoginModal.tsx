...
export default function AdminLoginModal({ isOpen, onClose }: AdminLoginModalProps) {
  ...
  const handlePasscodeCheck = (value: string) => {
    if (blocked) return;
    setPasscode(value);
  };
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={passcode}
                    onChange={(e) => handlePasscodeCheck(e.target.value)}
                    disabled={blocked}
                    className="w-full bg-[#111] border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white focus:outline-none focus:border-primary transition-colors disabled:opacity-50"
                  />
                </div>

                {error && <p className="text-red-500 text-sm">{error}</p>}

                {!blocked && ADMIN_PASSCODE && passcode === ADMIN_PASSCODE && (
                  <motion.button
