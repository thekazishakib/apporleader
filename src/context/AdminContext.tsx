...
export const AdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [slides, setSlidesState] = useState<SlideItem[]>([]);
  ...
  const fetchAllData = async () => {
    const [membersRes, leadershipRes, galleryRes, eventsRes, blogsRes, settingsRes, videosRes, slidesRes] = await Promise.all([
      ...
      supabase.from('slides').select('*').order('class_number', { ascending: true }),
    ]);
  ...
  const setSlides = (newSlides: SlideItem[]) => {
    ...
    syncCollection('slides', slides, newSlides);
  };
