...
export default function Members() {
  const { isAdmin, members, setMembers, leadershipTeam, setLeadershipTeam, siteSettings, authReady } = useAdmin();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const sanitized = {
      ...
      twitter: sanitizeUrl(formData.twitter),
      facebook: sanitizeUrl(formData.facebook),
      instagram: sanitizeUrl(formData.instagram),
      gmail: sanitizeText(formData.gmail, 100),
    };
    const isLeaderRole = sanitized.role.toLowerCase().includes('president') || sanitized.role.toLowerCase().includes('founder');
    ...
    if (editingId) {
      ...
      if (editingSource === 'leadership') {
        ...
        setLeadershipTeam(leadershipTeam.map((m: any) => m.id === editingId ? { ...sanitized, id: editingId, isPermanent: permanent } : m));
      } else {
        setMembers(members.map((m: any) => m.id === editingId ? { ...sanitized, id: editingId } : m));
      }
    } else {
      if (isLeaderRole) {
        setLeadershipTeam([...leadershipTeam, { ...sanitized, id: Date.now().toString(), isPermanent: false }]);
      } else {
        setMembers([...members, { ...sanitized, id: Date.now().toString() }]);
      }
    }
    setIsEditing(false);
  };
  ...
  return (
    <div className="w-full bg-[#0a0a0a] min-h-screen">
      <Helmet>
        ...
        <meta name="description" content="Meet the talented members and ambassadors of the ApporLeader community — united by logic, AI, and a drive to excel." />
        <link rel="canonical" href="https://apporleader.vercel.app/members" />
        <meta property="og:type" content="website" />
        <meta property="og:site_name" content="ApporLeader" />
        <meta property="og:title" content="Members | ApporLeader" />
        <meta property="og:description" content="Meet the talented members and ambassadors of the ApporLeader community." />
        <meta property="og:url" content="https://apporleader.vercel.app/members" />
        <meta property="og:image" content="https://apporleader.vercel.app/og-image.png" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Members | ApporLeader" />
        <meta name="twitter:description" content="Meet the talented members and ambassadors of the ApporLeader community." />
